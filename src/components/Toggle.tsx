...
const SlideToggle: React.FC<SlideToggleProps> = ({
  ...
  return (
    <SlideToggleContext.Provider value={{ toggled }}>
      <motion.button
        className="relative flex h-[25px] max-w-[45px] cursor-pointer items-center overflow-hidden rounded-full bg-gray-600/50 outline-offset-2 duration-200 focus-within:outline-red-500/70"
        ...
        <AnimatePresence initial={false}>
          {toggled && (
            <motion.span
              ...
              className="absolute left-0 flex w-full items-center justify-center rounded-full bg-gray-800"
            />
          )}
        </AnimatePresence>
        <motion.span
